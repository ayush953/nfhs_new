...
@Component({
  selector: 'app-eligible-man-response-rate-memberwise',
  templateUrl: './eligible-man-response-rate-memberwise.component.html',
  styleUrls: ['./eligible-man-response-rate-memberwise.component.scss']
})
export class EligibleManResponseRateMemberwiseComponent implements OnInit{
  constructor(
...
exportColumns :any[]=[];
  ...
  ngOnInit() {
    ...
    this.householdService.getAllEligibleMenResponseRateMemberwise(this.stateId,this.teamNo).subscribe(
      (datalist) => {
...
exportPdf() {
  import('jspdf').then((jsPDF) => {
      import('jspdf-autotable').then((x) => {
          ...
          doc.save('Eligible-man-response-rate-memberwise.pdf');
      });
  });
}
...
saveAsExcelFile(buffer: any, fileName: string): void {
  ...
  FileSaver.saveAs(data, fileName + '_Eligible-man-response-rate-memberwise_' + new Date().getTime() + EXCEL_EXTENSION);
}
