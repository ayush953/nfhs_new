...
@Component({
  selector: 'app-childrens-weight-heaping-second-decimal',
  templateUrl: './childrens-weight-heaping-second-decimal.component.html',
  styleUrls: ['./childrens-weight-heaping-second-decimal.component.scss']
})
export class ChildrensWeightHeapingSecondDecimalComponent 
{
  ...
  async getdata(){
     this.datalist = await lastValueFrom(this.householdService.getAllChildrensWeightHeapingSecondDecimal());
  }
...
exportPdf() {
    import('jspdf').then((jsPDF) => {
        import('jspdf-autotable').then((x) => {
            ...
            doc.save('childrens-weight-heaping-second-decimal.pdf');
        });
    });
}
...
saveAsExcelFile(buffer: any, fileName: string): void 
{
    ...
    FileSaver.saveAs(data, fileName + '_childrens-weight-heaping-second-decimal_' + new Date().getTime() + EXCEL_EXTENSION);
 }
