...
@Component({
  selector: 'app-vaccination-health-cards-memberwise',
  templateUrl: './vaccination-health-cards-memberwise.component.html',
  styleUrls: ['./vaccination-health-cards-memberwise.component.scss']
})
export class VaccinationHealthCardsMemberwiseComponent implements OnInit{
  constructor(
...
exportColumns :any[]=[];
  ...
  ngOnInit() {
   ...
   this.cols = [                
    ...
    this.householdService.getAllVaccinationHealthCardsMemberwise(this.stateId,this.teamNo).subscribe(
      (datalist) => {
...
exportPdf() {
  import('jspdf').then((jsPDF) => {
      import('jspdf-autotable').then((x) => {
          ...
          doc.save('Vaccination-Health-Cards-memberwise.pdf');
      });
  });
}
